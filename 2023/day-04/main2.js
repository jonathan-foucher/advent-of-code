...
const cards = readFile(FILE_NAME)
  .map(line => {
    const winningNumbers = line.split(':')[1].split('|')[0].split(' ').filter(str => str).map(str => parseInt(str))
    const numbers = line.split(':')[1].split('|')[1].split(' ').filter(str => str).map(str => parseInt(str))
