...
for (let i = 0; i < aunts.length; i++) {
  ...
  for (let j = 0; j < aunt.list.length; j++) {
    ...
    if ((GREATER_KEYS.includes(element.key) && TARGET[element.key] >= element.value)
      || (LESSER_KEYS.includes(element.key) && TARGET[element.key] <= element.value)
      || (!GREATER_KEYS.includes(element.key) && !LESSER_KEYS.includes(element.key) && TARGET[element.key] !== element.value)
    ) {
      isPossible = false
      break
    }
