...
const getMaxCost = (cost, damage, armor, weaponId, armorId, ringIds) => {
  let results = []
  ...
  if (!weaponId) {
    ...
    for (let i = 0; i < items.length; i++) {
      const item = items[i]
      results.push(getMaxCost(item.cost, item.damage, item.armor, item.id, null, []))
    }
  } else {
    if (!armorId) {
      ...
      for (let i = 0; i < items.length; i++) {
        const item = items[i]
        results.push(getMaxCost(cost + item.cost, damage + item.damage, armor + item.armor, weaponId, item.id, ringIds))
      }
    }
    ...
    if (ringIds.length < 2) {
      ...
      for (let i = 0; i < items.length; i++) {
        const item = items[i]
        results.push(
          getMaxCost(cost + item.cost, damage + item.damage, armor + item.armor, weaponId, armorId, [
            ...ringIds,
            item.id,
          ])
        )
...
const result = getMaxCost(0, 0, 0, 0, null, null, [])
