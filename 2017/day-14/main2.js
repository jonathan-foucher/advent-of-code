...
for (let row = 0; row < SIZE; row++) {
  ...
  const rowValue = hash
    .map((char) => parseInt(char, 16).toString(2).padStart(8, '0'))
    .join('')
    .split('')
  rowValues.push(rowValue)
}

const removeGroup = (x, y, rowValues) => {
  if (x >= 0 && y >= 0 && x < SIZE && y < SIZE && rowValues[y][x] === '1') {
    rowValues[y][x] = 0
    removeGroup(x - 1, y, rowValues)
    removeGroup(x, y - 1, rowValues)
    removeGroup(x + 1, y, rowValues)
    removeGroup(x, y + 1, rowValues)
  }
}
