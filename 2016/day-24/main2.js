...
const getMinimumTotalSteps = (currentTarget, visitedTargets, steps) => {
  if (visitedTargets.length === TARGETS.length) {
    const stepsToStart = STEP_RESULTS.find(
      (stepResult) => stepResult.targets.includes(START_TARGET) && stepResult.targets.includes(currentTarget)
    ).steps
    return steps + stepsToStart
  }
